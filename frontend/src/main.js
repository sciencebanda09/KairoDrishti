...
app.innerHTML = `
  ...
  <main class="workspace">
    ...
    <aside class="right-rail">
      ...
      <section class="event-card"><div class="card-head"><span class="section-kicker">EVENT LOG</span><div class="event-actions"><button class="quiet-button" id="startCamera">START CAMERA</button><button class="quiet-button" id="triggerDetection">TEST EVENT</button></div></div><div id="eventLog" class="event-log"><div class="event"><time>00:00</time><span class="event-marker neutral"></span><p><b>Mission initialized</b><small>Planner route loaded · return reserve protected</small></p></div></div></section>
    </aside>
    <footer class="footer-bar"><div class="timeline"><div class="timeline-labels"><span>MISSION TIMELINE</span><span id="phaseLabel">ROUTE LOADED</span><span>RTH RESERVE</span></div><div class="timeline-track"><i id="timelineProgress"></i></div></div><div class="footer-actions"><button id="toggleMission" class="primary">START MISSION</button><button id="restartMission">RESTART</button><button id="uploadDataset">UPLOAD DATASET</button><button id="exportPacket">EXPORT PACKET</button><input id="datasetInput" type="file" accept=".zip" hidden /></div></footer>
  </main>
`

const $ = (id) => document.querySelector(`#${id}`)
const state = { data: null, route: [], running: false, time: 0, speedMultiplier: 1, detected: false, detecting: false, resumed: false, layers: { heat: true, coverage: true, nfz: true }, frame: 0, liveStream: null, liveTimer: null }
...
function local(point) { return new THREE.Vector3((point.lon - state.data.home.lon) * 95000, (point.altitude || 0) - 1160, (point.lat - state.data.home.lat) * 111000) }
...
function updateDetectionMarkers() {
  ...
  worldObjects.detection = new THREE.Group()
  for (const detection of state.data.detections || []) {
    const p = local(detection)
    const marker = new THREE.Mesh(new THREE.SphereGeometry(4, 16, 10), new THREE.MeshStandardMaterial({ color: 0xc65b4c, emissive: 0x4a1713 }))
    marker.position.set(p.x, terrainY(p.x, p.z) + 6, p.z); worldObjects.detection.add(marker)
    const beam = new THREE.Mesh(new THREE.CylinderGeometry(.45, .45, 42, 10), new THREE.MeshBasicMaterial({ color: 0xc65b4c, transparent: true, opacity: .55 }))
    beam.position.set(p.x, terrainY(p.x, p.z) + 25, p.z); worldObjects.detection.add(beam)
    const label = makeLabel(`${detection.label.toUpperCase()} ${Math.round(detection.confidence * 100)}%`, 0xc65b4c); label.position.set(p.x - 18, terrainY(p.x, p.z) + 11, p.z); worldObjects.detection.add(label)
  }
  world.add(worldObjects.detection)
}
...
function triggerDetection() { if (state.detected || state.detecting) return; state.detecting = true; const canvas = document.createElement('canvas'); canvas.width = 96; canvas.height = 96; const ctx = canvas.getContext('2d'); ctx.fillStyle = '#101010'; ctx.fillRect(0, 0, 96, 96); ctx.fillStyle = '#f5f5f5'; ctx.fillRect(42, 34, 12, 28); canvas.toBlob((blob) => blob && sendFrame(blob).catch((error) => { state.detecting = false; addEvent('Test frame rejected', error.message, 'alert') }), 'image/jpeg', .95) }
function applyMission(payload) { state.data = payload; state.route = payload.waypoints.slice(); state.detected = (payload.detections || []).length > 0; state.detecting = false; updateRoute(); updateDetectionMarkers(); if (state.detected) { $('sensorMode').textContent = 'THERMAL CONFIRMED'; addEvent('Detection accepted', `${payload.detections.length} candidate(s) · route version ${payload.route_version}`, 'alert'); state.time = 0; state.running = true; $('toggleMission').textContent = 'PAUSE MISSION' } }
function telemetryMetadata() { const p = getPosition(); return JSON.stringify({ frame_id: `live-${state.frame}`, lat: state.data.home.lat + p.z / 111000, lon: state.data.home.lon + p.x / 95000, altitude_m: 1200 + p.y, heading_deg: 0, ground_sample_distance_m: .25 }) }
async function sendFrame(blob, sensor = 'rgb') { const form = new FormData(); form.append('image', blob, `frame-${state.frame}.jpg`); form.append('sensor', sensor); form.append('metadata', telemetryMetadata()); const response = await fetch('/api/frames/detect', { method: 'POST', body: form }); if (!response.ok) throw new Error(await response.text()); applyMission(await response.json()) }
async function startLiveCamera() { if (state.liveStream) { state.liveStream.getTracks().forEach((track) => track.stop()); state.liveStream = null; clearInterval(state.liveTimer); $('startCamera').textContent = 'START CAMERA'; addEvent('Live camera stopped', 'No new frames are being sent'); return } try { const video = document.createElement('video'); video.autoplay = true; video.muted = true; state.liveStream = await navigator.mediaDevices.getUserMedia({ video: { width: 640, height: 480 }, audio: false }); video.srcObject = state.liveStream; await video.play(); const canvas = document.createElement('canvas'); canvas.width = 640; canvas.height = 480; const ctx = canvas.getContext('2d'); state.liveTimer = setInterval(() => { ctx.drawImage(video, 0, 0, canvas.width, canvas.height); canvas.toBlob((blob) => blob && sendFrame(blob).catch((error) => addEvent('Frame rejected', error.message, 'alert')), 'image/jpeg', .82) }, 1200); $('startCamera').textContent = 'STOP CAMERA'; addEvent('Live RGB camera connected', 'Frames are sent to the local detector every 1.2 seconds', 'complete') } catch (error) { addEvent('Camera unavailable', error.message, 'alert') } }
async function analyzeDataset(file) { const form = new FormData(); form.append('dataset', file); const response = await fetch('/api/dataset/analyze', { method: 'POST', body: form }); if (!response.ok) throw new Error(await response.text()); applyMission(await response.json()); addEvent('Dataset analyzed', `${file.name} · ${state.data.detections?.length || 0} detection(s)`, 'complete') }
...
function resetMission() { state.time = 0; state.detected = false; state.detecting = false; state.running = false; state.route = state.data.waypoints.slice(); $('toggleMission').textContent = 'START MISSION'; $('eventLog').innerHTML = '<div class="event"><time>00:00</time><span class="event-marker neutral"></span><p><b>Mission initialized</b><small>Planner route loaded · return reserve protected</small></p></div>'; $('sensorMode').textContent = 'RGB / THERMAL'; updateRoute(); updateDetectionMarkers() }

function animate() { requestAnimationFrame(animate); if (state.running) { state.time += 0.055 * state.speedMultiplier; if (!state.detected && !state.detecting && state.time > 34) triggerDetection(); if (state.time > state.route.length * 18 + 12) { state.running = false; $('toggleMission').textContent = 'START MISSION'; addEvent('Return to base complete', 'Mission ended with reserve available', 'complete') } } moveDrone(); uiUpdate(); updateOrbit(); renderer.render(scene, camera); renderCameraFeed() }
...
function initButtons() { $('toggleMission').onclick = () => { state.running = !state.running; $('toggleMission').textContent = state.running ? 'PAUSE MISSION' : 'RESUME MISSION' }; $('restartMission').onclick = resetMission; $('triggerDetection').onclick = triggerDetection; $('startCamera').onclick = startLiveCamera; $('uploadDataset').onclick = () => $('datasetInput').click(); $('datasetInput').onchange = () => $('datasetInput').files[0] && analyzeDataset($('datasetInput').files[0]).catch((error) => addEvent('Dataset rejected', error.message, 'alert')); $('exportPacket').onclick = exportPacket; $('resetView').onclick = () => { orbit = { theta: 0.7, phi: 1.05, radius: 260, down: false, x: 0, y: 0 } }; $('toggleHeat').onclick = () => { state.layers.heat = !state.layers.heat; worldObjects.priority.forEach((x) => x.visible = state.layers.heat); $('toggleHeat').classList.toggle('selected', state.layers.heat) }; $('toggleNfz').onclick = () => { state.layers.nfz = !state.layers.nfz; worldObjects.noFly.visible = state.layers.nfz; $('toggleNfz').classList.toggle('selected', state.layers.nfz) }; $('toggleCoverage').onclick = () => { state.layers.coverage = !state.layers.coverage; $('toggleCoverage').classList.toggle('selected', state.layers.coverage) } }

async function boot() { state.data = await fetch('/api/mission').then((r) => r.json()); state.route = state.data.waypoints.slice(); $('missionId').textContent = state.data.mission_id; buildScene(); buildMissionLayers(); updateDetectionMarkers(); initInput(); initButtons(); updateCamera(); animate() }
