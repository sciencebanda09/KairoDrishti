...
test('synthetic flight advances through route coordinates', async ({ page }) => {
  ...
  await expect(page.locator('#cameraMode')).toHaveText('RGB VIEW')
  await expect(page.locator('#cameraBandOverlay')).toBeHidden()
...
test('dashboard keeps the live camera beside the map and operations below', async ({ page }) => {
  const mapBox = await page.locator('.dashboard-top .viewport-panel').boundingBox()
  const cameraBox = await page.locator('.camera-stage').boundingBox()
  const operationsBox = await page.locator('#operationsGrid').boundingBox()
  expect(mapBox?.width).toBeGreaterThan(500)
  expect(cameraBox?.width).toBeGreaterThan(300)
  expect(cameraBox?.x).toBeGreaterThan(mapBox?.x || 0)
  expect(operationsBox?.y).toBeGreaterThan((mapBox?.y || 0) + (mapBox?.height || 0) - 10)
  await expect(page.locator('.operations-grid #missionCard')).toBeVisible()
  await expect(page.locator('.operations-grid #detectionPanel')).toBeVisible()
})
...
test('mission guidance explains the operational loop', async ({ page }) => {
  ...
  await expect(page.locator('#tracksPanel')).toBeVisible()
  await expect(page.locator('#tracksEmpty')).toContainText('AWAITING REPEATED SIGHTINGS')
})

test('human-sign intelligence counts detections from an uploaded image', async ({ page }) => {
  await expect(page.locator('#detectionIntelligence')).toBeVisible()
  await expect(page.locator('#count-person')).toHaveText('0')
  await expect(page.locator('#count-clothing')).toHaveText('0')
  await expect(page.locator('#count-shelter')).toHaveText('0')
  await expect(page.locator('#count-track')).toHaveText('0')
  await page.locator('#imageInput').setInputFiles({ name: 'search-frame.jpg', mimeType: 'image/jpeg', buffer: Buffer.from('fixture') })
  await expect(page.locator('#detectionAnalysisState')).toHaveText('CUES FOUND')
  await expect(page.locator('#count-person')).toHaveText('1')
...
test('candidate detection becomes a rescue coordinate packet', async ({ page }) => {
  await page.getByRole('button', { name: 'RUN SENSOR CHECK' }).click()
  await expect(page.locator('#detectionPanel')).toContainText('POSSIBLE PERSON')
...
test('analysis bands change the active camera presentation', async ({ page }) => {
  ...
  await expect(page.locator('#cameraMode')).toHaveText('THERMAL VIEW')
  await expect(page.locator('#cameraBandOverlay')).toBeHidden()
  await page.getByRole('tab', { name: 'CHANGE' }).click()
  await expect(page.locator('.camera-card')).toHaveClass(/band-change/)
  await expect(page.locator('#cameraMode')).toHaveText('CHANGE PASS REVIEW')
  await expect(page.locator('#cameraBandOverlay')).toBeHidden()
...
test('swarm control shows four drones, hatch, and an associated track after two passes', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'REPLAY PASS' }).click()
  await page.getByRole('button', { name: 'REPLAY PASS' }).click()
  await expect(page.locator('.track-row')).toHaveCount(1)
  await expect(page.locator('.track-row')).toContainText('TRK-001')
  await expect(page.locator('#airspaceBadge')).toHaveText('SWARM · PLANNING MODE')
